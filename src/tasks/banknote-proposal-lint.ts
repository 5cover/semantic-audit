...
export const banknoteProposalLint = defineAuditTask({
  ...
  schemas: {
    ...
    summaryPayload: z.strictObject({
      ...
      reviewed_scene_specifications: z.number().int().min(0),
      choice_overload_scenes: z.number().int().min(0),
      instruction_polarity: z.strictObject({
        positive_requirements: z.number().int().min(0),
        negative_restrictions: z.number().int().min(0),
